...
int	check_dead(t_philo *philo)
{
	...
	i = -1;
	while (++i < philo->num_philo) //philo[0].
	{
		if (philo_die(&philo[i], philo[i].time_to_die) == 1) //1 is true
		{
			philo_say("died", &philo[i], philo[i].philo_id);
			pthread_mutex_lock(philo->dead_lock); //philo[0].
			*philo->die = 1;
			pthread_mutex_unlock(philo->dead_lock); //philo[0].
			return (1);
		}
	}
	return (0);
...
int	check_done_eating(t_philo *philo)
{
	...
	if (philo->num_for_philo_eat == -1) //philo[0].
		return (0);
	while (++i < philo->num_philo) //philo[0].
	{
		pthread_mutex_lock(philo->meal_lock);
